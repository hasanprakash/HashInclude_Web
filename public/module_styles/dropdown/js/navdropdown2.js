(function ($) {
  ...
  var $$ = (function () {
    ...
    Item.prototype.children = function (deepSearch) {
      ...
      $(this).each(function () {
        ...
        $items.each(function () {
          if (
            (deepSearch && !this.offsetWidth && !this.offsetHeight) ||
            this.disabled ||
            $(this).is(Selector.DATA_BUTTON) ||
            $(this).hasClass(ClassName.DISABLED) ||
            ~$.inArray(this, elements)
          ) {
            return;
          }
      ...
      while ((selector = selectors.pop())) {
        switch (selector) {
          ...
          case ":closed":
            if (
              (selector == ":opened") !=
              $this.parent().hasClass(ClassName.OPEN)
            )
              return false;
          case ":toggle":
  ...
  NavDropdown._clearMenus = function (event) {
    ...
    if (event.target) {
      if (this === document) {
        if (
          $(event.target).is("a:not([disabled], ." + ClassName.DISABLED + ")")
        ) {
          ...
          var $rootNode =
            (event.targetWrapper &&
              $(event.targetWrapper).find(Selector.BASE)) ||
            $(event.target).closest(Selector.BASE);
        ...
        if ($(event.target).hasClass(ClassName.BACKDROP)) {
          ...
          if (
            $nextNode.is(Selector.BASE) &&
            NavDropdown._isCollapsed($nextNode)
          ) {
            return;
          }
        }
      ...
      if ($(event.target).is(Selector.DATA_TOGGLE)) {
        filter = $(event.target.parentNode)
          .parents(Selector.DROPDOWN)
          .find(">" + Selector.DATA_TOGGLE);
      } else {
    ...
    var toggles =
      (event.data &&
        event.data.toggles &&
        $(event.data.toggles).parent().find(Selector.DATA_TOGGLE)) ||
      $.makeArray($(Selector.DATA_TOGGLE).not(filter));
  ...
  NavDropdown._dataApiKeydownHandler = function (event) {
    ...
    if (event.which == Hotkeys.ESC) {
      ...
      var toggle = $(event.target)
        .parents(Selector.DROPDOWN + "." + ClassName.OPEN)
        .last()
        .find(">" + Selector.DATA_TOGGLE);
      NavDropdown._clearMenus();
    ...
    if ($$item.is(":flat")) {
      ...
      if (event.which === Hotkeys.LEFT || event.which === Hotkeys.RIGHT) {
        ...
        (event.which === Hotkeys.DOWN || event.which === Hotkeys.UP) &&
        $$item.is(":toggle")
      ) {
        $$item
          .children()
          [event.which === Hotkeys.DOWN ? "first" : "last"]()
          .focus();
      }
    } else {
  ...
  NavDropdown._isCollapsed = function (rootNode) {
    ...
    return (
      rootNode &&
      (match = /navbar-toggleable-(xs|sm|md|lg|xl)/.exec(rootNode.className)) &&
  ...
  $(window).on(
    Event.RESIZE_DATA_API + " " + Event.LOAD_DATA_API,
    NavDropdown._dataApiResizeHandler
  );
  ...
  $(document)
    .on(
      ...
      Selector.BASE,
      NavDropdown._dataApiKeydownHandler
    )
