$(document).ready(function() {
    var date1 = (new Date('2/4/2022'));
    var date2 = new Date();
    var timeDiff = Math.abs(date2 - date1);
    var daysDiff = ("0" + Math.floor(timeDiff / (1000 * 60 * 60 * 24))).slice(-2)
    var hoursDiff = ("0" + (Math.floor(timeDiff / (1000 * 60 * 60))%24)).slice(-2)
    var minutesDiff = ("0" + ((Math.floor(timeDiff / (1000 * 60)))%60)).slice(-2)
    var secondsDiff = ("0" + ((Math.floor(timeDiff / (1000)))%60)).slice(-2)
    var time = daysDiff+":"+hoursDiff+":"+minutesDiff+":"+secondsDiff;
    var flip = new FlipClock({
      isCountdown: true,
      startTime: time,
      containerElement: $('.clock-countdown'),
      face: {
        days: {
          maxValue: 31
        },
  ...
  FlipClock.prototype.getDefaultConfig = function() {
      return {
          ...
          maxTime: '23:59:59',
          minTime: '00:00:00',
          containerElement: $('.clock-container'),
